...
const VisionContent = ({ setSearchWord, close }) => {
  ...
  return (
    <div>
      ...
      <div >
        <input hidden type="file" id="file" accept="image/*" onChange={(e) => setInputImage(URL.createObjectURL(e.target.files[0]))} />
        <label htmlFor="file">
          <div style={{ display: "flex", padding: "16px 0px" }}>
            <i className="fa-solid fa-image fa-lg"></i>
            <div style={{ lineHeight: '6px', paddingLeft: '4px' }}>사진 첨부</div>
          </div>
        </label>

      </div >
      <Cropper
        style={{ height: "100%", width: "100%", maxWidth: "360px", maxHeight: "360px" }}
        src={inputImage}
        crop={onCrop}
        ref={cropperRef}
        viewMode={1}
      ...
      <FlexDiv>
        ...
        <FlexDiv style={{ flexDirection: 'column', justifyContent: 'flex-end' }}>
          <TextDiv>필요한 부분을 자른 후</TextDiv>
          <TextDiv>검색 버튼 클릭</TextDiv>
          <CustomBtn onClick={requestHandler} className='search'>검색</CustomBtn>
        </FlexDiv>
      </FlexDiv>
    </div >
